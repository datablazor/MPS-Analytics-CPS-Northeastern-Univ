...
## Longnose dace is a freshwater minnow seen in the N. American rivers
...
## Source: https://stats.libretexts.org/Bookshelves/Applied_Statistics/Book%3A_Biological_Statistics_(McDonald)/05%3A_Tests_for_Multiple_Measurement_Variables/5.05%3A_Multiple_Regression
## Maryland Biological Stream Survey

# dependent variable = number of longnose dace per 75m  section of stream
# independent variables = 
# area (in acres) drained by the stream; 
# the dissolved oxygen (in mg/liter); 
# the maximum depth (in cm) of the  75m  segment of stream; 
# nitrate concentration (mg/liter); 
# sulfate concentration (mg/liter); 
# the water temperature on the sampling date (in degrees C)
...
dt = data.table(read.table(textConnection(Input),header=TRUE))
str(dt)
summary(dt)

dt.num = dt[, names(dt)[unlist(lapply(dt, is.numeric))], with = F]
...
chart.Correlation(dt.num,
                  method="pearson",
                  histogram=TRUE,
                  pch=20)

## Model selection using the step function (direction can be forward, backward or both)
# Uses AIC as a selection criterion

## A full model and a null are defined, and then the function will follow a procedure to find the model with the lowest AIC
## AIC measures the goodness of fit between two models - lower the AIC better the model
